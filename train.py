...
txt = tk.Entry(window,width=20  ,bg="gray25" ,fg="white",font=('times', 27, ' bold '))
txt.place(x=400, y=205)

lbl2 = tk.Label(window, text="Nhập tên",width=20 ,fg="white"  ,bg="gray25"    ,height=2 ,font=('times', 15, ' bold '))
lbl2.place(x=100, y=300)

txt2 = tk.Entry(window,width=20  ,bg="gray25"  ,fg="white",font=('times', 27, ' bold ')  )
txt2.place(x=400, y=305)
...
def TakeImages():
    ...
    if(is_number(Id) and name.isalpha()):
        ...
        sampleNum=0
        while(True):
            ret, img = cam.read() #frame: data camera
            gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY) # convert to gray picture
